...
APP_VERSION = "0.26.1"               # CFBundleShortVersionString 과 일치해야 한다
...
#
# **우리 스폰이 갱신을 일으킨다 — 측정됐다. 다만 전 구간이 아니다.** 한때 여기 "아직
# 관측되지 않았다"고 적혀 있던 전제이고, 이제 절반은 사실로 확인됐다. 두 쪽을 붙여 읽되
# 섞지는 말 것:
#
#   **관측된 것** (2026-09-20, 윈도우). 외부 워처 — Claude Code 밖에서, WMI 로 띄워
#   세션 트리 밖에 둔 pythonw — 가 무인으로 헤드리스 `claude -p /usage` 를 한 번 띄웠고
#   (exit 0, 1.84초), **스폰 +0.78초에 자격증명 파일이 다시 쓰였다**: accessToken 과
#   refreshToken 지문이 둘 다 바뀌고 만료가 −0.173h → +7.975h 로 옮겨 갔다(재시도 0회).
#   귀속도 깨끗하다 — 스폰 직전 샘플과 이후 2초 간격 45개 샘플(88.3초) 전부
#   `claude_pids: []` 라, 그 구간에 갱신을 대신 해 줬을 다른 claude.exe 가 없었다.
#   같은 시각 생긴 트랜스크립트 크기(4519 B)도 알려진 `claude -p` 프로브 크기(4494 B)와
#   거의 같아 교차 확인된다.
#
#   **관측되지 않은 것.** 그 실험의 발화 조건은 `expiresAt + 10분` 이었다. 즉 검증된
#   명제는 "**이미 만료된** 토큰에 대해 우리 스폰이 0.8초 안에 갱신을 유발한다" 이고,
#   **이 기능이 실제로 발화하는 지점인 만료 120초 *전*은 직접 검증되지 않았다.** 그 사전
#   구간도 될 것이라고 볼 근거는 있다 — CLI 자신의 갱신 마진이 262초로 측정됐으니
#   T−120초에 띄운 CLI 는 시작하자마자 자기 마진 안이라고 판단할 것이다. 하지만 그것은
#   **추론이지 측정이 아니다.** 사전 구간이 다르게 행동하는 것으로 드러나면 선제 경로는
#   값을 잃지만, 반응 경로(만료 후)는 위에서 그대로 측정된 그 경로이고 가드도 그대로다.

# 만료 이 시간 전부터 선제 갱신을 건다. **CLI 보다 늦게 도착하도록** 고른 값이다.
#
# 2026-09-20, 윈도우, n=2. **상주하는 Claude Code 세션은 만료 4분 안팎에 스스로 갱신한다:**
#   · 외부 워처가 528 샘플·8시간 45분·공백 0건으로 지켜본 세션 — 8시간 동안 입력도 API
#     호출도 없이 완전히 idle 이었는데 **만료 262초(4.37분) 전**에 갱신했다.
#   · 같은 날 05:42:37 의 두 번째 사례 — 원래 만료 3.7~4.7분 전이고, 그 시점 센서스에
#     상주 claude.exe 가 있었다.
# CLI 의 마진이 정확히 얼마인지는 이 두 표본으로 알 수 없다(단정하지 말 것). 아는 것은
# "만료 4분 안팎 지점에는 이미 갱신했다" 뿐이고, 그것으로 충분하다.
#
# 300 은 두 사례 어느 쪽보다도 이르다(첫 사례보다 38초, 두 번째의 이른 끝보다도 앞선다).
# 그 값으로는 Claude Code 를 쓰는 사용자마다 CLI 가 어차피 할 갱신을 우리가 매번 가로채
# Node 앱을 한 번 더 띄우게 된다(윈도우에선 한 번에 프로세스 트리 최대 7개). 그래서 120
# 으로 내린다 — 두 사례 중 **가장 이른** 지점(3.7분 ≈ 222초)에서도 100초 넘게 뒤라,
# 관측이 1분 남짓 틀렸더라도 경쟁이 되살아나지 않는다.
#
...
def recovery_spawn_argv(cli_path):
    ...
    만료 **전** 구간은 아직 추론이다 — 위 모듈 주석의 '관측된 것 / 관측되지 않은 것' 을
    그대로 읽을 것. 출력을 게이지에 연결하지는 않는다 — 수치는 OAuth API 가 계속 맡는다.
    CLI 의 수치 줄은
