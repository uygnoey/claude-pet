# `.claude_pet/pets` — 예시 커스텀 펫 세트

이 폴더는 **claude-pet(v0.18)** 에서 바로 쓸 수 있는 완성된 커스텀 펫 예시 모음입니다.
...
> 참고: 구형 포맷도 지원됩니다 — `pet.json` 없이 `idle/`, `running/` 등 **상태별 하위 폴더 안의 PNG 시퀀스**로도 펫을 만들 수 있습니다(앱 내장 고양이가 이 방식). 신규 펫은 위 시트 방식을 권장합니다.
