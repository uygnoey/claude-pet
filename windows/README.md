...
**레지스트리에 하나가 더 남습니다**: `HKCU\…\Explorer\StartupApproved\Run` 의 `ClaudePet` 값입니다. 제거 프로그램의
`uninsdeletevalue` 도, 앱 안 "완전 삭제…" 의 `delete_run_value_if_ours` 도 `Run` 값만 지우고 Explorer 의 승인 기록은
건드리지 않습니다. 승인할 `Run` 값이 없으면 아무 효력이 없고 다시 설치하면 "사용" 으로 맞아떨어져 해는 없지만,
"완전 삭제 뒤 남는 것은 `.claude_pet\` 뿐" 은 엄밀히 틀린 문장입니다 — 자세한 것과 다음 릴리즈 후보라는 점은
아래 "실기에서 확인할 것" 의 해당 항목에 적어 두었습니다.
...
시작 앱 UI 에서 끈 것이 그대로 꺼진 것으로 보이고, 앱이 몰래 다시 켜지 않습니다. 그 UI 는 끌 때 Run 값을 지우지
않고 `HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\StartupApproved\Run` 의 같은 이름 REG_BINARY 값에 기록합니다 —
**설정 › 앱 › 시작 앱과 작업 관리자 › 시작 앱이 같은 키에 쓰는 두 화면**이므로, 이 기록의 출처를 둘 중 하나로만 적으면 틀립니다.

**그 값의 규칙은 첫 바이트의 최하위 비트입니다 — 짝수면 사용, 홀수면 사용 안 함.** 즉 `0x00` 과 `0x02` 는 사용,
`0x01` 과 `0x03` 은 사용 안 함입니다. 근거를 관찰·그 관찰이 가리키는 방식·아직 추정인 것으로 나눠 적습니다(AGENTS.md §5):

- **관찰(기기 한 대, Windows 11, 2026-09-14, 시작 앱 UI 에서 껐다 켬)**: 시작 상태가 다른 **두 쌍**을 같은 UI 에서 얻었습니다.
  기록이 없던 상태에서는 "사용 안 함" 이 `01 00 00 00` + 8바이트 FILETIME(첫 바이트 `0x01`), 다시 "사용" 이 12바이트 0
  (첫 바이트 `0x00`) 이었고, 기록이 이미 `02 00 00 00 00 …` 이던 상태에서는 "사용 안 함" 이 `0x03`, 다시 "사용" 이 `0x02`
  였습니다. FILETIME 이 붙는 쪽은 끌 때이고, 켤 때는 첫 바이트 뒤가 전부 0 입니다. 그 기기의 타사 항목 **여덟 개**는
  이 UI 가 건드린 적이 없고 모두 `0x02` 였습니다.
- **그 두 쌍이 가리키는 방식**: Windows 가 기존 기록의 상위 비트는 그대로 두고 최하위 비트만 뒤집는다는 것. 한 규칙으로
  `0x00`/`0x01` 과 `0x02`/`0x03` 이 함께 설명되고, 그것이 코드가 이미 구현하고 있는 규칙이라 이 관찰은 짝수/홀수 구현을
  **확인해 줍니다**(바꿀 것이 없습니다). 다만 기기 한 대의 동작에서 끌어낸 추론이지 문서화된 계약이 아니고, 다른 사람이
  재현하지도 않았습니다 — 달라진 것은, 더 이상 값 한 쌍에서 끌어낸 맨 추정이 아니라는 점입니다. `0x02` ↔ `0x03` 은
  `0x00`/`0x01` 이 아닌 첫 바이트가 같은 비트로 움직이는 것을 직접 본 자리입니다. (기기 한 대는 여전히 **반증**까지만 합니다.
  실제로 예전 코드의 "`0x02` = 사용, `0x03` = 사용 안 함" 은 이 관찰로 반증됐습니다 — 사용 상태인 `0x00` 을 사용 안 함으로
  읽어, Windows 는 띄우는데 메뉴 항목만 체크가 없었습니다.)
- **아직 추정**: `0x04`(사용) / `0x05`(사용 안 함) 처럼 `0x03` 위의 첫 바이트. 그런 값을 가진 기록은 본 적이 없고, 관찰이 아니라
  위 방식에서 따라 나오는 것뿐입니다. 나중 실기 결과가 이 부분만 뒤집을 수 있습니다.
...
- **관찰(기기 한 대, Windows 11, 2026-09-14)**: **화면은 `설정 › 앱 › 시작 앱`** 이다(작업 관리자도, 설정 › 앱 → 설치된 앱도
  아니다 — 이번 실기에서 확정됐다). 거기서 이름은 `ClaudePet.exe`, 게시자는 빈칸이었고 바로 위 Claude 항목은
  "Claude / Anthropic, PBC" 였다. 고친 뒤 같은 줄은 "Claude Pet / Yeongyu Yang" 으로 보인다.
- **관찰과 무관하게 참인 것**: 시작 앱 목록(설정 › 앱 › 시작 앱, 작업 관리자 › 시작 앱·세부 정보)은 exe 의
  `FileDescription`/`CompanyName` 을 읽고, 설정 › 앱 → 설치된 앱은 Inno 의 ARP 값을 읽는다. 뒤쪽의 `AppPublisher` 는
  `installer.iss` 에 이미 있으므로 거기서 게시자가 비어 있을 수는 없다. 화면이 확정된 지금도 고치는 자리는 같다 —
  exe 의 버전 리소스다.
...
- **실기 2026-09-14 (고침)**: 진짜 설치본에서 `install_kind()` 가 `inno` 를 돌려주는가(제거 레지스트리 키의 닫는 중괄호 둘 —
  위 "업데이트"). 완전 삭제 확인 창이 inno 계획(제거 프로그램 실행)을 보여 주는지로도 눈에 보인다 — **이 두 번째 문장은
  틀렸다. 이 목록 맨 아래 "정정" 항목을 보라.**
...
- **실기 2026-09-14 (고침, 화면 확정)**: "Claude Pet / Yeongyu Yang" 으로 뜨는가(exe 버전 리소스). **화면은 `설정 › 앱 › 시작 앱`
  이었다** — 작업 관리자도, 설정 › 앱 → 설치된 앱도 아니었고, 고친 뒤 그 줄이 "Claude Pet / Yeongyu Yang" 으로 보였다
  (위 "exe 의 버전 리소스"). 지난 보고에 "설정/작업 관리자" 까지만 남아 있던 자리가 이것으로 메워졌다. 남은 것은 다른 두
  목록(작업 관리자 › 시작 앱·세부 정보, 설정 › 앱 → 설치된 앱)에서도 같은 이름·게시자로 보이는지와, 파일 속성 › 자세히 탭의
  `FileVersion` 이 `APP_VERSION` 과 같은지다.
...
- 완전 삭제(inno/portable) 뒤 남는 것: 파일은 `%USERPROFILE%\.claude_pet\` 뿐이고(펫을 닫아 놓고 제거했을 때는 실기
  2026-09-14 에서 확인됐다 — `.claude_pet\` 과 `.claude_pet.json` 만 남았다. 펫이 떠 있는 채로도 같은지가 이번에 고친 부분이다),
  **레지스트리에는 `HKCU\…\Explorer\StartupApproved\Run` 의 `ClaudePet` 값이 남는다**(실기 2026-09-14, 마지막 제거 뒤
  보고된 바이트는 `00 00 00…` — 첫 바이트 `0x00`). `installer.iss` 의 `uninsdeletevalue` 는 `Run` 값만 지우고 Explorer 의 승인 기록은
  건드리지 않으며, 앱 안 "완전 삭제…" 도 `delete_run_value_if_ours` 로 `Run` 값만 지운다 — 두 경로 다 이 값을 남긴다.
  **해는 없다**: 승인할 `Run` 값이 없으면 아무 효력이 없고, 다시 설치하면 `0x00`(짝수 = 사용)이라 그대로 "사용" 으로 맞아
  떨어진다. 그래도 "완전 삭제 뒤 남는 것은 `.claude_pet\` 뿐" 이라고 적으면 엄밀히 틀리므로 여기에 적어 둔다. 이 값까지
  지우는 것은 **다음 릴리즈 후보**다 — 이번 릴리즈에서는 `installer.iss` 를 바꾸지 않는다. 다음에 손댈 때는 두 경로
  (설치 파일의 제거 프로그램, 앱 안 "완전 삭제…") 를 함께 봐야 한다. `Run` 값과 달리 이 값의 내용에는 경로가 없어
  "우리 것인가" 를 값 이름(`ClaudePet`) 말고는 물을 데가 없다는 점도 그때 같이 정해야 한다.
...
- 로그인 시 자동 실행: 설치 파일에서 옵션을 켠 뒤 우클릭 메뉴 항목에 체크가 있는가; 항목을 끄고 켜면 작업 관리자 › 시작 앱의
  ...
  값 첫 바이트는, 기록이 없던 상태에서 껐다 켜면 사용 안 함 `0x01` / 사용 `0x00`, 기록이 `02 00 00 00 00 …` 이던 상태에서
  껐다 켜면 사용 안 함 `0x03` / 사용 `0x02` 였다. 그 기기의 타사 항목 여덟 개는 모두 `0x02`. 두 쌍이 함께 가리키는 것은
  Windows 가 상위 비트를 보존하고 최하위 비트만 뒤집는다는 것이고, 코드의 짝수 = 사용 / 홀수 = 사용 안 함은 그대로다
  (위 "로그인 시 자동 실행"). 남은 **추정**은 `0x03` **위의** 첫 바이트뿐이니, 그런 값이 실제로 보이면
  `win_autostart.startup_approved_enabled` 와 그 시험 표를 함께 고친다. 다시 볼 때는 작업 관리자와 설정 › 앱 › 시작 앱
  **양쪽에서** 껐다 켜 보고 — 두 화면이 같은 키에 쓰므로 어느 쪽에서 껐는지도 함께 적는다 — 그 기기에 있는 타사 항목
  전부의 첫 바이트를 판정이 아니라 **raw 바이트 그대로** 적는다. `0x01`/`0x03` 이 아닌 홀수나 `0x00`/`0x02` 가 아닌
  짝수가 보이는 것이 남은 추정을 확인하거나 뒤집는 자리다. 보고된 상태(Run 값 있음 + StartupApproved `00 ×12`)에서 메뉴 항목에
  ...
  없는가(같은 이름의 StartupApproved 값은 남는다 — 위 "완전 삭제 … 뒤 남는 것").
- **정정(2026-09-14)**: 위 `install_kind()` 항목에 적혀 있던 기대 — "완전 삭제 확인 창이 inno 계획(제거 프로그램 실행)을
  보여 주는지로도 눈에 보인다" — 은 **성립할 수 없다**. 지우지 않고 적어 두는 이유는 이 목록이 실기 세션에게 무엇을 보라고
  보낸 기록이기 때문이다. 확인 창은 계획의 `("delete", …)` 단계만 줄로 만들고(`claude_pet_win._uninstall`), 종류 단계
  (`("run", [unins000.exe, "/SILENT"])` 또는 `("helper", exe_dir)`)는 화면에 오르지 않는다. 게다가 `_uninstall` 은 `source`
  가 아닌 모든 종류에서 `exe_dir` 을 목록 맨 앞에 끼워 넣으므로, inno 와 portable 의 확인 창은 **같은 줄들**을 보여 준다 —
  창만 보고는 두 종류를 구별할 수 없다. 판별자는 `install_kind()` 자체이고, 그것은 직접 재야 한다: 실기에서는 제거·업데이트
  뒤 `update.log` 의 `kind=` 필드(`uninstall kind=inno …`, `install … kind=inno …`, `check status=update … kind=…`)를 읽거나,
  설치 폴더 옆의 `unins000.exe` 와 제거 레지스트리 키의 `InstallLocation` 이 그 폴더를 가리키는지를 직접 확인한다.
