...
**v0.21**
- 세션·주간·모델 3개 게이지를 Claude 앱에 보이는 %로 보정합니다. 2M을 쓴 상태에서 25%를 입력하면 한도가 8M으로 잡히고, 빈 칸은 기존 한도를 유지합니다.
- 직접 입력하는 3개 M 단위 한도는 고급 창에 남아 있고 현재 값이 함께 표시됩니다. 같은 항목에 %와 M을 모두 넣으면 %가 우선합니다.
- 0%는 역산할 수 없으니 빈 칸으로 두거나 사용량이 쌓인 뒤 다시 저장하세요. 정확 모드 게이지는 서버 값이지만 급증 감지는 보정한 추정 한도를 쓰며, 추정 계산은 그대로라 다시 보정할 필요 없고 안내는 4개 언어로 제공합니다.
