...
**v0.16**
- **키체인 허용 프롬프트가 아예 안 뜨고 정확 모드가 안 되던 문제** 수정 — 키체인이 프롬프트를 띄우지 못하고 조용히 실패하는 상태(`-25308`/`-25315`)일 때 폴백 경로가 없어, 2분마다 같은 실패만 반복하며 영영 정확 모드로 넘어가지 못했습니다. 이제 사용자가 **직접 거부한 게 아니면** 항상 `security` 폴백으로 한 번 더 시도합니다. 토큰이 파일에 있는 설치본은 키체인을 아예 건드리지 않아 프롬프트 없이 바로 정확 모드가 됩니다.
