...
- 필이 제공자별로 줄을 나누고, 각 제공자의 수치가 그 로고와 함께 자기 줄에 보입니다 — Claude만 써도 Claude 로고가 생깁니다. Codex는 계정에 있는 한도만 주간·세션 이름으로 나타나고, 쓰지 않으면 줄도 로고도 생기지 않습니다.
- 크레딧을 켜 두었다면 쓴 금액이 $로 보입니다. 우클릭 메뉴 "크레딧 금액으로"에서 %로 바꿀 수 있고, API 모드에서 키가 거부되면 "조회 중"에 머물지 않고 이유를 알려 줍니다.
...
- 펫 위의 표시가 요약 필 하나로 통일됐습니다. 첫 줄은 세션·주간·모델(Fable 등) 사용량 %, 둘째 줄은 리셋 시각이고, API 모드에서는 오늘·이번 달 비용을 월 예산과 함께 보여 줍니다.
- 색이 뜻을 알려 줍니다. 수치는 출처(정확 모드 에메랄드, 로그 추정 앰버·값 앞 ≈, API 비용 코랄)이고, 라벨은 흰색이다가 50%부터 노란색, 85%부터 빨간색이며 급증하면 ▲와 함께 빨간색, 토큰이 만료돼 추정으로 내려가면 줄 끝에 ⚠ 입니다.
- 요약 글꼴로 Pretendard (OFL)를 앱에 내장했고, zip을 풀어 한 단계 더 깊어진 펫 폴더(pets/이름/이름/)도 인식하며, Windows용 시험판(claude-pet-win.zip, 서명 없음)이 처음으로 함께 올라갑니다. 기존 사용자는 따로 할 일이 없고, 접기/펴기·크기 조절·설정은 그대로입니다.
